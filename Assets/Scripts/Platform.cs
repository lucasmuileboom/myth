...
public class Platform : MonoBehaviour
{
	...
	void Update ()
    {
        if (_Player.transform.position.y - _playerHeight > transform.position.y + _platformHeight && !_goDown)
        {
        ...
        else if (_Player.transform.position.y - _playerHeight < transform.position.y)
        {
            _goDown = false;
            GetComponent<BoxCollider2D>().enabled = false;
            gameObject.layer = 0;
